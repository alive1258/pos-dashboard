...
import { useGetAllSaleReportQuery } from "@/redux/api/saleSummariesApi";
...
const SaleReport = () => {
  ...
  const { data, error, isLoading, refetch } = useGetAllSaleReportQuery(query);
  const [deleteProduct] = useDeleteProductMutation();

  // Filters companies based on the search query
  const filteredData = data?.data?.data;
  console.log(filteredData, "filteredData sale report");
      ...
      <div className="bg-[#0D0E12]  top-[75px] md:top-[82px] z-[450] py-2 md:py-0">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 ">
          <div className="w-full">
            <SectionTitle
              big_title={"Sale Report"}
              link_one={"/"}
              title_one={"Home"}
              title_two={"Product Sale Report"}
              link_two={"/sale-system/sale-report"}
            />
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="table_row ">
                  ...
                  <th className="table_th">Total Quantity</th>
                  ...
                  filteredData.map((item, index) => (
                    <tr key={index} className="tbody_tr">
                      ...
                      <td className="table_th ">
                        <div className="w-36">
                          <p>{truncateCharacters(item?.customer_name, 30)}</p>
                        </div>
                      </td>
                      <td className="table_th">
                        <div className="w-36">
                          <p>{truncateCharacters(item?.customer_phone, 30)}</p>
                        </div>
                      </td>
                      <td className={`table_th  `}>
                      ...
                      <td className="my-2 px-4 text-center ">
                        <div className="flex items-center justify-center w-full gap-2">
                          <EditIcon
                            edit_link={`/sale-system/sale-report/${item?.id}`}
                          />
                        </div>
                      </td>
                    </tr>
